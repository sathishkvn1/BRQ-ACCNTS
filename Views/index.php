...
<body class="hold-transition sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed">
						...
						<div class="col-md-6 px-4 border-right index_left">
							<div class="row">
								<div class="col-lg-12 text-center mt-2">
									...
									<div><strong>1-Apr-17 to 31-Mar-18</strong></div>
								</div>
							</div>
								...
								<div class="col-lg-8 offset-lg-2">
									<div class="card card-current-date">
										...
										<div class="card-body">
											...
											<ul class="card-title mainMenuIndex">
												<h6>MASTERS</h6>
												<li class="card-text selected"><a href="#" id="create_master"
												...
												<li class="card-text"><a href="#" data-toggle="modal"
														...
														tabindex="-1" data-backdrop="static"><span>C</span>reate(new)</a></li>
												<li class="card-text"><a href="#" tabindex="-1"><span>A</span>lter</a>
												</li>
												<li class="card-text"><a href="#" tabindex="-1">C<span>H</span>art of
														Accounts</a></li>
												...
												<li class="card-text"><a href="#" tabindex="-1">Day
														Boo<span>K</span></a></li>

												<h6>UTLITIES</h6>
												<li class="card-text"><a href="#" tabindex="-1">Ba<span>N</span>king</a>
												</li>

												<h6>REPORTS</h6>
												<li class="card-text"><a href="#" tabindex="-1"><span>B</span>alance
														Sheet</a></li>
	...
	<div class="modal fade bottom modal-old-vouchers masters vouchers_menu" id="vouchers" role="dialog"
		...
		<div class="modal-dialog modal-full-height modal-bottom">
			...
			<div class="modal-content">
				<div class="modal-header">
					<div>List Of Vouchers</div>
				</div>
				<div class="modal-body">
					<div class="row">
						<div class="col-md-12 key-control-menu">
							<ul class="mainMenu">
								<li class="card-text selected"><a
	...
	<div class="modal fade bottom modal-old-vouchers masters" id="masters" role="dialog" data-backdrop="static"
		data-keyboard="false">
		<div class="modal-dialog modal-full-height modal-bottom">
	...
	<div class="modal fade bottom modal-create-side-new" id="masters_two" role="dialog" data-backdrop="static"
		data-keyboard="false">
		<div class="modal-dialog modal-full-height modal-bottom">
							...
							<div class="wrapper">
								<div class="outer_div">
									<ul class="ul_movement">
										...
										<li class="masters_list_side_heading" id="accounting_masters"
										...
										<li class="masters_list_side_heading " id="master_inventory_masters"
										...
										<li class="masters_list_side_heading" id="master_payroll_masters"
										...
										<li class="masters_list_side_heading" id="master_statutory_master"
										...
										<li class="masters_list_side_heading" id="master_statutory_detailes"
											data-value="li_statutory_detailes">Statutory
										Detailes</li>
										<li class="element li_statutory_detailes" id="master_gst_detailes"><a
												href='#'  onclick="openGstModal()">GST Detailes</a>
										</li>
...
<script>
var li = $('.mainMenuIndex li');
	var liSelectedClass = $('.mainMenuIndex li.selected');
	var liSelected;

	$(window).on("keyup", function (e) {
		var scrollli = $('.ul_movement li');
			var scrollliSelectedClass = $('.ul_movement li.element-hover');
			var scrollliSelected;
		if ($("#masters_two").hasClass("show")) {//the section work when you open list of master page 
			
			if (e.which === 40) {//key down 

				if (scrollliSelectedClass) {
					scrollliSelectedClass.removeClass('element-hover');
					scrollnext = scrollliSelectedClass.next();
					// if (!scrollnext.hasClass('element')) {
					// 	scrollnext = scrollnext.next();
					// } 
					if ($(scrollnext).css("display") == "none") {

						scrollnext = scrollnext.nextAll(".masters_list_side_heading:first");

					}
					if (scrollnext.length > 0) {
						scrollliSelectedClass = scrollnext.addClass('element-hover');
					}
					else {

						scrollliSelectedClass = scrollli.eq(0).addClass('element-hover');
					}
				}
				else {
					scrollliSelectedClass = scrollli.eq(0).removeClass('element-hover');
					liSelectedClass = li.eq(1).addClass('element-hover');
				}
			}
			else if (e.which === 38) {//key up
				if (scrollliSelectedClass) {
					scrollliSelectedClass.removeClass('element-hover');
					scrollprev = scrollliSelectedClass.prev();
					// if (!scrollprev.hasClass('element')) {
					// 	scrollprev = scrollprev.prev();
					// }
					if ($(scrollprev).css("display") == "none") {
						scrollprev = scrollprev.prevAll(".masters_list_side_heading:first");
					}
					if (scrollprev.length > 0) {
						scrollliSelectedClass = scrollprev.addClass('element-hover');
					}
					else {
						scrollliSelectedClass = scrollli.last().addClass('element-hover');
					}
				}
				else {
					scrollliSelectedClass = scrollliSelectedClass.prev().addClass('element-hover');
					scrollliSelectedClass = scrollliSelectedClass.removeClass('element-hover');
				}
			}
			
			else if(e.which === 13){//eneter key 
				var id_of_selected_element=$(".element-hover").attr("id");
				if($("#"+id_of_selected_element).hasClass("masters_list_side_heading"))	//hide and show the li items when you press enter key on headding of group li
					{
				...
				{
					$("#id_of_selected_element").click();
				}
			}
			$(".wrapper .ul_movement").scrollTop(0);//set to top
			$(".wrapper .ul_movement").scrollTop($('.element-hover:first').offset().top - $(".wrapper .ul_movement").height());//then set equal to the position of the selected element minus the height of scrolling div
			

		}
		else if(!$(".modal").hasClass('show')) {// key movement for gate way of brq erp 9
			if (e.which === 40) {
	...
	$(".masters_list_side_heading").dblclick(function () {
